#!/bin/bash
echo $ENV_CONFIG_HC_CUSTOMER | base64 --decode > /src/public/.env
...
_init_xdebug() {
  ...
  if [[ $_xdebug_enableb == 1 ]] ; then
    ln -svf /etc/php/7.2/mods-available/xdebug.ini /etc/php/7.2/cli/conf.d/20-xdebug.ini
    ln -svf /etc/php/7.2/mods-available/xdebug.ini /etc/php/7.2/fpm/conf.d/20-xdebug.ini
  fi
}
...
_init_newrelic() {
  ...
  if [[ $_newrelic_enableb == 1 ]] ; then
    local _f_conf="/etc/php/7.2/mods-available/newrelic.ini"
    ...
    ln -svf "$_f_conf" /etc/php/7.2/cli/conf.d/20-newrelic.ini
    ln -svf "$_f_conf" /etc/php/7.2/fpm/conf.d/20-newrelic.ini
  fi
}
